...
这样仍然走的是正式 `run_residual_training.py` 的 learner 代码，不是 test benchmark 脚本，但可以避免 actor / env / RPC 噪声干扰。
...
1. 在正式 `run_residual_training.py` learner 路径里，打开 bf16 的收益是明确正的
