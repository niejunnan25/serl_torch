...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
    ...
    echo
    echo "Standard AgiBot actor wrapper for scripts/run_residual_training.py."
    echo "Default config: configs/train_residual.yaml"
    echo "Before launching on the robot terminal, run: source robot/service/env.sh"
    echo "Example:"
    echo "  bash tools/run_actor.sh policy.type=joyra policy.port=9001"
    exit 0
fi
