...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
    echo "Usage: bash tools/run_learner_canonical.sh [hydra overrides...]"
    echo
    echo "Canonical AgiBot learner wrapper for scripts/run_residual_training.py."
    echo "Default config: configs/train_residual.yaml"
    echo "Example:"
    echo "  bash tools/run_learner_canonical.sh training.max_update_steps=10000"
    exit 0
fi
