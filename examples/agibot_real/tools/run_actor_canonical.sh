...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
    echo "Usage: bash tools/run_actor_canonical.sh [hydra overrides...]"
    echo
    echo "Canonical AgiBot actor wrapper for scripts/run_residual_training.py."
    echo "Default config: configs/train_residual.yaml"
    echo "Example:"
    echo "  bash tools/run_actor_canonical.sh policy.type=joyra policy.port=9001"
    exit 0
fi
