...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -h|--help)
            echo "Usage: bash tools/run_learner_generic.sh <yaml|path/to/config.yaml> [--gpu_id N] [extra overrides...]"
            echo
            echo "Config-driven standalone learner wrapper."
            echo "This wrapper does not force training.async.* settings."
            echo "Note: the current scripts/train/run_learner.py entrypoint still requires"
            echo "training.async.enabled=true and training.async.backend=agentlace."
            echo "Default split-process workflow: tools/run_learner.sh or tools/run_learner_agentlace.sh"
            exit 0
            ;;
        --gpu_id|--gpu-id)
            require_arg "$1" "${2:-}"
...
cd "$ROOT_DIR"
echo "=========================================="
echo "  AgiBot Generic Learner Wrapper"
echo "=========================================="
echo "  Mode        : config-driven"
echo "  Async mode  : from config/overrides"
echo "  Note        : current learner entrypoint is still agentlace-only"
echo "=========================================="
