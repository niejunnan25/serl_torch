...
class VRTeleoperationController:
    ...
    def _calculate_relative_motion(self, current_vr_pose, hand='right'):
        ...
        # 如果正在初始化或未启用相对运动，返回零增量
        if state['is_initializing'] or not state['relative_motion_enabled'] or state['last_vr_position'] is None:
    ...
    def _process_controller_input(self, hand, position, rotvec, button_states):
        ...
        if button_states[grip_key] and not state['is_button_pressed']:
            ...
            state['init_frame_count'] = 0
            state['relative_motion_enabled'] = False  # 等待初始化完成
        ...
        # 初始化阶段：收集10帧数据以稳定IMU零飘
        if state['is_button_pressed'] and state['is_initializing']:
            state['init_frame_count'] += 1
            # 当收集到足够的帧数后，设置初始位姿
            if state['init_frame_count'] >= state['init_buffer_size']:
                state['is_initializing'] = False
        ...
        if state['relative_motion_enabled']:
            ...
            with self.position_lock:
                #防止重定位跳变
                # if np.linalg.norm(state['initial_position'] + pos_diff - state['current_position']) < 0.05:
    ...
    def get_control_signals(self):
        ...
        with self.position_lock:
            return {
                'timestamp': self.delta_control_signals['timestamp'].copy(),
                # 右手柄数据
