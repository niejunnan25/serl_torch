"""Keyboard-driven fake AgiBot task env with the real step_chunk contract."""
...
class AgiBotFakeTaskEnv:
    """AgiBot task-env substitute for interface and rollout smoke tests."""
    ...
    def __init__(
        ...
        *,
        ...
        self._fake_pose = self._initial_fake_pose()
        self._last_robot_action = np.asarray(self._fake_pose, dtype=np.float32).copy()
    ...
    def _initial_fake_pose(self) -> np.ndarray:
        pose = np.zeros((AGIBOT_ROBOT_ACTION_DIM,), dtype=np.float32)
        pose[6] = 0.5
        pose[13] = 0.5
        return pose
    ...
    def _validate_action_chunk(self, actions: np.ndarray) -> np.ndarray:
    ...
    def _logical_action_to_robot_action(self, action: np.ndarray) -> np.ndarray:
        return embed_logical_action(action, self._fake_pose, self._arm_layout)

    def _apply_logical_action(self, action: np.ndarray) -> np.ndarray:
        robot_action = self._logical_action_to_robot_action(action)
        self._last_robot_action = np.asarray(robot_action, dtype=np.float32).copy()
        self._fake_pose = np.asarray(robot_action, dtype=np.float32).copy()
        return robot_action
    ...
    def _get_obs(self) -> Dict[str, np.ndarray]:
        ...
        def _cam(seed: int) -> np.ndarray:
            ...
            return np.clip(
                layer.astype(np.int16) + noise.astype(np.int16),
                0,
                255,
            ).astype(np.uint8)
        ...
        obs: Dict[str, Any] = {
            ...
            "state/pose": np.asarray(self._fake_pose, dtype=np.float32).copy(),
            "state/head": np.array(
                [0.01 * np.sin(phase), 0.01 * np.cos(phase)],
                dtype=np.float32,
            ),
            "state/waist": np.array(
                [0.005 * np.sin(phase * 0.9), 0.005 * np.cos(phase * 0.9)],
                dtype=np.float32,
            ),
        }
        self._last_obs = obs
    ...
    def _prearm_controller_for_dry_run(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "fake env pre-armed controller to RUNNING before first reset"
            )
    ...
    def _controller_loop(self) -> None:
        ...
        while not self._control_stop.is_set():
            ...
            if queued is not None:
                try:
                    self._apply_logical_action(queued.action)
    ...
    def _resolve_step_result(
        ...
        *,
        ...
        info_dict: Dict[str, Any] = {
            ...
            "task_description": self._task_description,
            "task_name": self.task_name,
            "arm_layout": str(self._arm_layout),
            "action_dim": int(self._action_dim),
            "robot_action_dim": int(self._robot_action_dim),
        }
        reward = 0.0
        done = False
        truncated = False
    ...
    def enqueue_action_chunk(self, actions: np.ndarray) -> list[int]:
        if not self._controller_enabled or self._controller is None:
            raise RuntimeError("controller mode is disabled")
        return self._controller.enqueue_action_chunk(self._validate_action_chunk(actions))
    ...
    def _controller_execute_chunk_blocking(
        ...
        assert self._controller is not None
        action_chunk = self._validate_action_chunk(actions)
    ...
    def step(
        ...
        action_chunk = self._validate_action_chunk(np.asarray(action, dtype=np.float32))
        if action_chunk.shape[0] != 1:
            raise ValueError(f"Expected one action for step(), got {action_chunk.shape}")
        if self._controller_enabled and self._controller is not None:
            results = self._controller_execute_chunk_blocking(action_chunk)
            if not results:
        ...
        self._apply_logical_action(action_chunk[0])
        self._take_action_cnt += 1
        obs = self._get_obs()
        step_result = self._resolve_step_result(
            obs=obs,
