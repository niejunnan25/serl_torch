...
def _run_expert_precheck(
    ...
    *,
    ...
    if not cfg.training.expert_check:
        return True, None
    passed, episode_info = env.expert_precheck(init_episode_idx=int(init_episode_idx))
    if not passed:
        logger.warning(
            "skip init_episode_idx=%s: expert precheck failed",
            int(init_episode_idx),
        )
    return bool(passed), episode_info
...
def actor(cfg: AgiBotTrainConfig, *, run_dir: Path, logger: logging.Logger) -> None:
    ...
    try:
        while env_steps < max_env_steps and episode_id < max_episodes:
            init_episode_idx = int(episode_cursor)
            episode_cursor += 1
            passed, episode_info = _run_expert_precheck(
                cfg,
                env=env,
                init_episode_idx=init_episode_idx,
                logger=logger,
            )
            if not passed:
                skipped_prechecks += 1
            ...
            logger.info(
                "episode=%s init_episode_idx=%s success=%s steps=%s return=%.3f env_steps=%s",
                int(episode_id),
                int(init_episode_idx),
        ...
        summary.update(
            {
                ...
                "skipped_prechecks": int(skipped_prechecks),
            }
        )
