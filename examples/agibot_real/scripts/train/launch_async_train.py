...
"""Hydra-native async stack launcher for AgiBot real residual SAC training."""
...
def main(cfg: DictConfig) -> None:
    ...
    logger = logging.getLogger("agibot_real_launch_async_train")

    repo_root = Path(__file__).resolve().parents[4]
    tools_dir = repo_root / "examples" / "agibot_real" / "tools"
    ...
    output_root = _resolve_path(
        launch_cfg.get("output_root", "outputs/agibot_real/launch_async_train"),
        base=original_cwd,
    )
    ...
    logger.info("Hydra run dir: %s", run_root)
    ...
    try:
        ...
        logger.warning(
            ...
            "If you want launch logs under launch.output_root, pass hydra.run.dir accordingly.",
            run_root,
            expected_run_parent,
        )
    ...
    if _port_open(trainer_port_check_host, trainer_port):
        raise RuntimeError(f"Trainer port already in use: {trainer_port_check_host}:{trainer_port}")
    ...
    try:
        ...
        env_proc = _launch_background(
            name="env server",
            cmd=["bash", str(tools_dir / "serve_env.sh"), "--host", env_host, "--port", str(env_port)],
            cwd=repo_root,
            log_path=env_log,
        )
        managed.append(env_proc)
        _wait_for_port(name="env server", host=env_host, port=env_port, timeout_sec=env_timeout, logger=logger)

        if async_eval_enabled:
            same_as_train_env = async_eval_env_host == env_host and async_eval_env_port == env_port
            if same_as_train_env:
                ...
                logger.info("Reusing existing async eval env server at %s:%s", async_eval_env_host, async_eval_env_port)
            else:
        ...
        openpi_proc = _launch_background(
            name="OpenPI server",
            cmd=["bash", str(tools_dir / "serve_openpi.sh"), "--port", str(openpi_port), "--gpu-id", str(openpi_gpu)],
            cwd=repo_root,
            log_path=openpi_log,
        )
        managed.append(openpi_proc)
        _wait_for_port(name="OpenPI server", host=openpi_host, port=openpi_port, timeout_sec=openpi_timeout, logger=logger)
