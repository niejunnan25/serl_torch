"""Observation-schema helpers for LIBERO residual training."""
...
def build_chunk_residual_obs(
    *,
    ...
    residual_obs: dict[str, np.ndarray] = {
        ...
        "base_action_chunk": np.expand_dims(base_actions, axis=0).astype(np.float32),
...
def prepare_base_actions_chunk(
    *,
    ...
    if base_actions_arr.ndim != 2:
        raise ValueError(
            f"{source} must return a 2D action chunk, got shape {base_actions_arr.shape}"
        )
    if int(base_actions_arr.shape[0]) < int(chunk_horizon):
        raise ValueError(
            f"{source} returned only {int(base_actions_arr.shape[0])} actions, "
            f"expected at least chunk_horizon={int(chunk_horizon)}"
        )
    return base_actions_arr[: int(chunk_horizon)]
