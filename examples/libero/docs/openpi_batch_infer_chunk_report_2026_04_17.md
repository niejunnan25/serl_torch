# OpenPI `infer_many` + LIBERO `chunk` 实施与实测记录

注：本文最初写于旧命名阶段；当时的 `copy_copy` / `optimized` 当前对应 `train_residual_chunk.py` / `chunk` 训练线。下文统一使用 `chunk`。
...
目标是把之前只在 JoyRA 路径可用的 chunk 级 batch backfill 能力，补到 OpenPI 上，让 `examples/libero/scripts/train_residual_chunk.py` 在 `policy.type=openpi` 时也能真正走：
...
`train_residual_chunk.py` 的 actor 仍然保持：
...
- `chunk` actor 的 `torch_compile` 语义 bug 已修。
...
- `chunk` learner 这次 summary 没有完全追上 actor 的 1000 env steps。
...
这个很关键，说明 `chunk` 的 post-hoc 组装开销已经被压得很低了。

### 4. 1000-step 端到端：原始 `train_residual_step.py`
...
这也很关键：原始 step-wise 训练线里，`build_decision_obs` 明显比 `chunk` 大很多。
...
- `chunk`: `5.803 step/s`
- `train_residual_step.py`: `5.105 step/s`
...
- `chunk` 现在在 OpenPI backend 下，确实能走 chunk 级 batched backfill
...
- 真实 1000-step 也跑通了
- `chunk` 的 `build_decision_obs` 平均耗时已经接近被清空
...
2. async backfill 的 episode-end drain 语义
   - 这次 `chunk` actor 已到 1000 steps，但 learner summary 没完全追平
