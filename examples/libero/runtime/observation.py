...
def build_fused_chunk_residual_observation(
    *,
    obs: dict[str, Any],
    ...
    if base_action_chunk_arr.ndim == 1:
        if action_dim is None:
            raise ValueError(
                "Flat base_action_chunk requires action_dim to infer chunk shape"
            )
        if int(action_dim) <= 0 or int(base_action_chunk_arr.size) % int(action_dim) != 0:
            raise ValueError(
                "Unexpected flat base_action_chunk size: "
                f"{base_action_chunk_arr.shape} for action_dim={action_dim}"
            )
        base_action_chunk_arr = base_action_chunk_arr.reshape(-1, int(action_dim))
    if base_action_chunk_arr.ndim != 2:
        raise ValueError(
            f"base_action_chunk must be 2D [H, A], got {base_action_chunk_arr.shape}"
        )
    if int(base_action_chunk_arr.shape[0]) <= 0:
        raise ValueError("base_action_chunk must include at least one step")
    if action_dim is not None and int(base_action_chunk_arr.shape[1]) != int(action_dim):
        raise ValueError(
            "Unexpected base_action_chunk action dim: "
            f"{base_action_chunk_arr.shape} vs (*, {int(action_dim)})"
        )
    if chunk_horizon is not None and int(base_action_chunk_arr.shape[0]) != int(chunk_horizon):
        raise ValueError(
            "Unexpected base_action_chunk horizon: "
            f"{base_action_chunk_arr.shape} vs ({int(chunk_horizon)}, *)"
        )

    base_action_arr = np.asarray(base_action_chunk_arr[0], dtype=np.float32).reshape(-1)
    fused_state = np.concatenate(
        (
            state_core,
            base_action_arr,
            base_action_chunk_arr.reshape(-1),
            np.asarray([residual_alpha], dtype=np.float32),
        ),
        axis=-1,
    ).astype(np.float32)
    ...
    residual_obs: dict[str, np.ndarray] = {
        "state": np.expand_dims(fused_state, axis=0).astype(np.float32),
        "base_action": np.expand_dims(base_action_arr, axis=0).astype(np.float32),
        "base_action_chunk": np.expand_dims(base_action_chunk_arr, axis=0).astype(
            np.float32
        ),
        "alpha": np.asarray([[residual_alpha]], dtype=np.float32),
    }
