...
"""Reusable LIBERO direct-action RLPD checkpoint evaluation runner."""
...
from serl_torch.examples.libero.rlpd.config import LiberoRLPDEvalConfig
...
def _resolve_checkpoint_input(
    ...
    *,
    ...
    if checkpoint_input_path.is_dir():
        ...
        resolved_checkpoint_path = checkpoint_input_path.resolve()
    return checkpoint_input_path, resolved_checkpoint_path


def run_eval(
    cfg: LiberoRLPDEvalConfig,
    *,
    run_dir: Path,
    ...
    require_eval_checkpoint(
        ...
        allow_random_policy=bool(cfg.eval.allow_random_policy),
    )
    ...
    if checkpoint_file is not None:
        ...
        logger.info(
            "Loaded direct-action checkpoint from: %s%s",
            checkpoint_file,
            (
                ""
        ...
        logger.warning(
            "eval.checkpoint_path is not set; running evaluation with the current in-memory policy state"
        )
