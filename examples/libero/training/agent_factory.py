...
def _vector_obs_keys(cfg: DictConfig) -> tuple[str, ...] | None:
    obs_cfg = cfg.get("obs", {})
    keys = obs_cfg.get("vector_obs_keys", None)
    if keys is None:
        return None
...
def _proprio_latent_dim(cfg: DictConfig) -> int:
    ...
    latent_dim = int(encoder_cfg.get("proprio_latent_dim", 64))
    if latent_dim <= 0:
        raise ValueError(
...
def make_drq_agent(
    ...
    *,
    ...
    if bool(encoder_cfg.get("use_proprio", False)) and vector_obs_keys is None and (
        "state" not in sample_obs
    ):
        raise ValueError(
            "encoder.use_proprio=true requires either observations['state'] or "
            "obs.vector_obs_keys to be configured"
        )
    ...
    kwargs: dict[str, Any] = {
        ...
        "encoder_type": str(encoder_cfg.get("type", "small")),
        "shared_encoder": bool(encoder_cfg.get("shared", True)),
        ...
        "critic_network_kwargs": {
            "activations": str(network_cfg.get("critic_activation", "relu")),
            "use_layer_norm": bool(network_cfg.get("critic_layer_norm", False)),
            "hidden_dims": _to_hidden_dims(
                network_cfg.get("critic_hidden_dims", [256, 256])
            ),
        },
        "policy_network_kwargs": {
            "activations": str(network_cfg.get("policy_activation", "relu")),
            "use_layer_norm": bool(network_cfg.get("policy_layer_norm", False)),
            "hidden_dims": _to_hidden_dims(
