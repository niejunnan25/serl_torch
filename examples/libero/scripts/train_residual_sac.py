...
from serl_torch.examples.libero.utils.train_loop_utils import (
    _chunk_bootstrap_mask,
    _current_train_env_step,
    _discounted_chunk_reward_sum,
    _insert_online_transition,
    _iter_period_hits,
    _pack_chunk_actions,
    _remaining_online_budget_steps,
)
...
def main(cfg: DictConfig) -> None:
    ...
    if chunk_step_scheduler_clock not in {"policy_step", "env_step"}:
        raise ValueError(
            f"chunk_step.scheduler_clock must be 'policy_step' or 'env_step', got {chunk_step_scheduler_clock}"
        )
    ...
    if offline_enabled:
        ...
        offline_residual_xi = _scheduled_xi(cfg, base_xi=residual_xi, global_policy_step=0)
        ...
        for phase in cfg.training.phases:
            ...
            while phase_episode_count < phase_episodes:
                ...
                while (episode_steps < max_episode_steps) and (not episode_done):
                    ...
                    if chunk_step_enabled:
                        train_env_step_before_chunk = _current_train_env_step(
                            warmup_separate=warmup_separate,
                            global_env_step=global_env_step,
                            residual_env_step=residual_env_step,
                        )
                        if chunk_step_scheduler_clock == "env_step":
                            schedule_step = train_env_step_before_chunk
                        else:
                            schedule_step = global_policy_step
                        ...
                        in_warmup_step = bool(
                            ...
                            and train_env_step_before_chunk < warmup_base_steps
                        )
                        if phase_train and (in_warmup_episode or in_warmup_step):
                            ...
                            random_chunk = np.random.uniform(
                                -1.0,
                                1.0,
                                size=(chunk_horizon, step_action_dim),
                            ).astype(np.float32)
                            residual_chunk = random_chunk
                        else:
                        ...
                        remaining_budget_steps = _remaining_online_budget_steps(
                            warmup_separate=warmup_separate,
                        ...
                        # Keep env-step semantics stable by preventing a chunk from crossing
                        # warmup/random boundaries in the middle.
                        if (
                            phase_train
                            and (not warmup_separate)
                            and warmup_base_steps > 0
                            and train_env_step_before_chunk < warmup_base_steps
                        ):
                            execute_horizon = int(
                                min(execute_horizon, warmup_base_steps - train_env_step_before_chunk)
                            )
                        elif (
                            phase_train
                            and train_env_step_before_chunk < int(cfg.training.random_steps)
                        ):
                            execute_horizon = int(
                                min(
                                    execute_horizon,
                        ...
                        if execute_horizon <= 0:
                            episode_done = True
                            break
                        current_policy_step = int(global_policy_step)
                        ...
                        for chunk_step in range(actual_chunk_steps):
                            ...
                            step_logger.write(
                                {
                                    "global_env_step": int(global_env_step),
                                    "global_policy_step": int(current_policy_step),
                                    "episode_id": episode_id,
                                    "phase": phase_name,
                                    "episode_step": episode_steps,
                                    "seed": int(env.last_seed if env.last_seed is not None else seed),
                        ...
                        train_env_step_after_chunk = _current_train_env_step(
                            warmup_separate=warmup_separate,
                            global_env_step=global_env_step,
                            residual_env_step=residual_env_step,
                        )
                        ...
                        should_update_now = bool(
                            phase_train
                            and _replay_progress_size(replay_buffer) >= int(cfg.training.training_starts)
                            and current_policy_step % int(cfg.training.update_every) == 0
                        )
                        ...
                        checkpoint_hits = _iter_period_hits(
                            ...
                            period=int(cfg.training.checkpoint_period),
                        )
                        ...
                        for chunk_step in range(chunk_horizon):
                            ...
                            train_env_step_before_step = _current_train_env_step(
                                warmup_separate=warmup_separate,
                                global_env_step=global_env_step,
                                residual_env_step=residual_env_step,
                            )
                            ...
                            in_warmup_step = bool(
                                not warmup_separate
                                and warmup_base_steps > 0
                                and train_env_step_before_step < warmup_base_steps
                            )
                            if phase_train and (in_warmup_episode or in_warmup_step):
                                residual_step_action = np.zeros((step_action_dim,), dtype=np.float32)
                            elif xi_step <= 0.0:
                            ...
                            if warmup_separate:
                                residual_env_step += 1
                            train_env_step_after_step = _current_train_env_step(
                                warmup_separate=warmup_separate,
                                global_env_step=global_env_step,
                                residual_env_step=residual_env_step,
                            )
                            next_xi_step = _scheduled_xi(
                                cfg,
                                base_xi=residual_xi,
                                global_policy_step=train_env_step_after_step,
                            )
                            episode_return += float(reward)
                            episode_success = bool(info["success"])
                            timeout = bool(episode_steps >= max_episode_steps)
                            if warmup_separate and max_online_env_steps > 0:
                                budget_exhausted = residual_env_step >= max_online_env_steps
                            else:
                                budget_exhausted = bool(
                                    max_online_env_steps > 0
                                    and global_env_step >= max_online_env_steps
                            ...
                            if (
                                ...
                                and train_env_step_after_step % int(cfg.training.checkpoint_period) == 0
                            ):
                                if async_learner is not None:
                                    async_learner.save_checkpoint(
                                        str(checkpoint_dir),
                                        step=int(train_env_step_after_step),
                                        keep=int(cfg.training.keep_checkpoints),
                                    )
                                else:
                                    checkpoint_payload = _snapshot_agent_checkpoint_payload(
