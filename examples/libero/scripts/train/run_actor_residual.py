...
from serl_torch.examples.libero.agent_factory import make_drq_agent
...
from serl_torch.examples.libero.residual_action import ResidualActionSpec
...
def actor(cfg: DictConfig, *, run_dir: Path, logger: logging.Logger) -> None:
    ...
    episode_seed = int(cfg.task.seed_base)
...
def learner(cfg: DictConfig, *, run_dir: Path, logger: logging.Logger) -> None:
    ...
    run_name = str(
        cfg.wandb.get(
            "exp_name",
            f"{cfg.task.suite_name}_task_{int(cfg.task.task_id)}_reference_style",
        )
    )
    wandb_cfg.update(
        {
...
def main(cfg: DictConfig) -> None:
    ...
    logger = logging.getLogger("libero_reference_style_residual")
    logger.info("Hydra run dir: %s", run_dir)
    logger.info("Config:\n%s", OmegaConf.to_yaml(cfg, resolve=True))

    set_global_seeds(int(cfg.seed))

    role = validate_reference_style_residual_cfg(cfg)
    if role == "actor":
