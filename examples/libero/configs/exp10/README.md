...
如果你只是想直接把 `exp10` 跑起来，最省事的方法是用封装脚本。
...
```bash
bash /vla/users/niejunnan/codebase/serl_torch/examples/libero/configs/exp10/COMMANDS.sh null launch
```
...
bash /vla/users/niejunnan/codebase/serl_torch/examples/libero/tools/launch_async_train.sh \
  /vla/users/niejunnan/codebase/serl_torch/examples/libero/configs/exp10/chunk/libero_10_task_8_chunk_async_null.yaml
```

常见变体：

```bash
bash /vla/users/niejunnan/codebase/serl_torch/examples/libero/configs/exp10/COMMANDS.sh null_utdeff1p launch
bash /vla/users/niejunnan/codebase/serl_torch/examples/libero/configs/exp10/COMMANDS.sh null_unfreeze launch
bash /vla/users/niejunnan/codebase/serl_torch/examples/libero/configs/exp10/COMMANDS.sh null_pi05 launch
```

说明：

- `launch` 模式会自动启动 `env / async_eval_env / openpi / learner / actor`
- 日志和输出会落到 Hydra run 目录下，默认在 `examples/libero/outputs/exp10/<config_name>/<timestamp>/`
...
CFG=$ROOT/examples/libero/configs/exp10/chunk/libero_10_task_8_chunk_async_null.yaml
RUN_ROOT=$ROOT/examples/libero/outputs/exp10/manual/null
...
bash examples/libero/tools/run_learner.sh \
  /vla/users/niejunnan/codebase/serl_torch/examples/libero/configs/exp10/chunk/libero_10_task_8_chunk_async_null.yaml \
  --bootstrap /vla/users/niejunnan/codebase/serl_torch/examples/libero/outputs/exp10/manual/null/agentlace_bootstrap.pkl \
  --gpu_id 1 \
  hydra.run.dir=/vla/users/niejunnan/codebase/serl_torch/examples/libero/outputs/exp10/manual/null/learner
```
...
bash examples/libero/tools/run_actor.sh \
  ...
  --bootstrap /vla/users/niejunnan/codebase/serl_torch/examples/libero/outputs/exp10/manual/null/agentlace_bootstrap.pkl \
  --gpu_id 0 \
  hydra.run.dir=/vla/users/niejunnan/codebase/serl_torch/examples/libero/outputs/exp10/manual/null/actor
```
...
- 一键启动 `pi05` 版本：`bash examples/libero/configs/exp10/COMMANDS.sh null_pi05 launch`
...
```bash
/vla/users/niejunnan/codebase/serl_torch/examples/libero/configs/exp10/FULL_COMMANDS.sh
```
