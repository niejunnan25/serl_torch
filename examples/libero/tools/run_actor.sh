...
usage() {
    cat <<'EOF'
Usage:
  bash tools/run_actor.sh <yaml_file_name.yaml|/abs/path/to/config.yaml> --bootstrap /abs/path/to/agentlace_bootstrap.pkl [--gpu_id N] [extra hydra overrides...]

Examples:
  bash tools/run_actor.sh train_residual_sac.yaml --bootstrap /abs/path/to/agentlace_bootstrap.pkl --gpu_id 0
EOF
}
...
exec "$PYTHON_BIN" "$ROOT_DIR/scripts/run_actor.py" \
