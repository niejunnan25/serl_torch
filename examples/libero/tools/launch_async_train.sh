...
usage() {
    cat <<'EOF'
Usage:
  bash tools/launch_async_train.sh <yaml_file_name.yaml|/abs/path/to/config.yaml> [hydra overrides...]

Examples:
  bash tools/launch_async_train.sh train_residual_sac.yaml
  bash tools/launch_async_train.sh /abs/path/to/config.yaml launch.actor_gpu=0 launch.learner_gpu=1
EOF
}
