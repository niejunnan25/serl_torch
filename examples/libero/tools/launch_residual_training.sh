...
Notes:
  ...
  - When the config name encodes an entrypoint, it must match the selected mode:
    ...
    Neutral config names are allowed and use the explicit --mode value.
...
if [[ -z "$CONFIG_TRAINING_MODE" ]]; then
    ...
    log_note "config '$CONFIG_COMPOSE_NAME' does not encode a training mode; using --mode $TRAINING_MODE"
fi
