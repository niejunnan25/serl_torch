...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -h|--help)
            cat <<'EOF'
Usage:
  bash tools/serve_openpi_policy.sh [--port PORT] [--gpu-id N] [--openpi-root DIR]
                                   [--policy-config NAME] [--policy-dir DIR]
                                   [--default-policy] [extra serve_policy args...]
EOF
            exit 0
            ;;
        --port)
...
echo "=========================================="
echo "  LIBERO OpenPI Policy Server"
echo "=========================================="
