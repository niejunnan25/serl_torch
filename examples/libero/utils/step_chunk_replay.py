...
class StepChunkReplayBuffer:
    ...
    def __init__(
        ...
        *,
        ...
        if self.state_core_dim <= 0:
            raise ValueError(
                f"state_core_dim must be positive, got {self.state_core_dim}"
            )
        if self.step_action_dim <= 0:
            raise ValueError(
                f"step_action_dim must be positive, got {self.step_action_dim}"
            )
        ...
        if not self.pad_action_to_horizon:
            raise ValueError(
                "StepChunkReplayBuffer currently requires pad_action_to_horizon=true"
            )
        ...
        if "base_action_chunk" not in stripped_template:
            raise ValueError(
                "sample_observation_template must include 'base_action_chunk' in chunk-step mode"
            )
        self._image_keys = tuple(
            key for key in stripped_template if key not in _OBS_SPECIAL_KEYS
        )
        ...
        self._images = {
            key: np.empty(
                (self.capacity, *stripped_template[key].shape),
                dtype=stripped_template[key].dtype,
            )
            for key in self._image_keys
        }
        self._state_core = np.empty(
    ...
    def _collect_segment(
        self, start_step_id: int, max_len: int
    ...
    def _obs_window_ready(self, start_step_id: int) -> bool:
        step_ids, terminal, full = self._collect_segment(
            start_step_id, self.chunk_horizon
        )
        if not step_ids:
            return False
        if full:
    ...
    def _refresh_candidate_window(self, last_inserted_step_id: int) -> None:
        ...
        start_step = max(
            self._min_active_step_id(), int(last_inserted_step_id) - lookback
        )
    ...
    def insert(self, data_dict: Dict[str, Any]) -> None:
        ...
        for key in self._image_keys:
            ...
            self._images[key][insert_index] = np.asarray(
                obs_core[key], dtype=self._images[key].dtype
            )
    ...
    def _assemble_obs(self, start_step_id: int) -> Dict[str, np.ndarray]:
        ...
        if not step_ids:
            raise RuntimeError(
                f"Unable to assemble observation for step_id={start_step_id}"
            )
        start_idx = self._buffer_index(start_step_id)
        ...
        for key in self._image_keys:
            obs[key] = np.expand_dims(
                np.array(self._images[key][start_idx], copy=True), axis=0
            )
        return obs
    ...
    def _build_transition(self, start_step_id: int) -> Dict[str, Any]:
        ...
        if not step_ids:
            raise RuntimeError(
                f"No chunk transition available for start_step_id={start_step_id}"
            )
        ...
        return {
            ...
            "masks": np.float32(
                0.0 if terminal else (self.discount ** max(0, chunk_steps - 1))
            ),
            "dones": bool(terminal),
    ...
    def sample(self, batch_size: int) -> Dict[str, Any]:
        ...
        return {
            "observations": {
                key: np.stack(
                    [transition["observations"][key] for transition in transitions],
                    axis=0,
                )
                for key in obs_keys
            },
            "actions": np.stack(
                [transition["actions"] for transition in transitions], axis=0
            ),
            "action_mask": np.stack(
                [transition["action_mask"] for transition in transitions], axis=0
            ),
            "next_observations": {
                key: np.stack(
                    [
                        transition["next_observations"][key]
            ...
            "rewards": np.asarray(
                [transition["rewards"] for transition in transitions], dtype=np.float32
            ),
            "masks": np.asarray(
                [transition["masks"] for transition in transitions], dtype=np.float32
            ),
            "dones": np.asarray(
                [transition["dones"] for transition in transitions], dtype=bool
            ),
            "chunk_steps": np.asarray(
                [transition["chunk_steps"] for transition in transitions],
                dtype=np.int32,
            ),
        }
